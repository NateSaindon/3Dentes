...
**This repo is public.** Keep personal, clinical, or identifying material out of
it — see "CBCT work" below before adding any imaging data.
...
The plan is to replace BodyParts3D hard tissue with a segmented CBCT volume the
user supplies. This is the main Phase 2 thrust and supersedes several options in
`docs/phase-2-options.md`. **Read [docs/cbct-plan.md](docs/cbct-plan.md) before
starting** — it covers what is and isn't extractable, the privacy decision that
must be made *before* the first commit of any derived mesh, and the first-session
steps.

Two things from it that matter most:
...
- **Nothing imaging-derived gets committed to this public repo** until the user
  has explicitly decided the visibility question. Dentition is forensically
  identifying and git history is unforgiving.
